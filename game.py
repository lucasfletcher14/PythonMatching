...
#DEFINE Tk AS SUPER VARIABLE FOR CONSTRUCTOR
class gameclass(Tk):
	def __init__(self):
		super().__init__()
	...
	#DISPLAY BLANK IMAGES IN 3x3 GRID
	def Label(self):
		...
		self.button2=Button(self,text="",command=lambda:self.check(2),borderwidth=1,height = 5, width = 17)
		self.button2.place(x=450,y=20)


		self.button3=Button(self,text="",command=lambda:self.check(3),borderwidth=1,height = 5, width = 17)
		self.button3.place(x=630,y=20)


		self.button4=Button(self,text="",command=lambda:self.check(4),borderwidth=1,height = 5, width = 17)
		self.button4.place(x=280,y=150)


		self.button5=Button(self,text="",command=lambda:self.check(5),borderwidth=1,height = 5, width = 17)
		self.button5.place(x=450,y=150)


		self.button6=Button(self,text="",command=lambda:self.check(6),borderwidth=1,height = 5, width = 17)
		self.button6.place(x=630,y=150)



		self.button7=Button(self,text="",command=lambda:self.check(7),borderwidth=1,height = 5, width = 17)
		self.button7.place(x=280,y=280)


		self.button8=Button(self,text="",command=lambda:self.check(8),borderwidth=1,height = 5, width = 17)
		self.button8.place(x=450,y=280)


		self.button9=Button(self,text="",command=lambda:self.check(9),borderwidth=1,height = 5, width = 17)
		self.button9.place(x=630,y=280)

		#ON CLICK, SHUFFLE THROUGH PHOTOS
	...
	def start_shuffle(self):
		if (self.number % 2)==0:
			...
			#RANDOMLY ASSIGN IMAGE TO SPACE
		...
		#HIDE THE IMAGES AND LET USER GUESS
		else:
			...
			self.Label1 = Label(self,text = "Animal ",font=("times",30),bg='white')
			self.Label1.place(x=274,y=450)
	...
	#DEFENSIVE PROGRAMMING; WON'T LET USER CLICK IMAGE WITHOUT STARTING
	def check(self,btnnmbr):
		if self.number_list==[]:
			messagebox.showinfo("Memory Game", "Lets start the Game first")
			return

		elif self.random_character=="":
			messagebox.showinfo("Memory Game", "Lets start the Game first")

		#CHECK IF THE CLICK WAS CORRECT ANSWER
		elif btnnmbr==1:
